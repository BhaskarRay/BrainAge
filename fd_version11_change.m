...
newFolder3 = fullfile(destpath_fd,'FWD_LOG_mp_tc');

destpath = '/data/mialab/users/bray14/FD_FMRI/';
newFolder = fullfile(destpath,'mp_final_file');
%newFolder2 = fullfile(destpath,'tc_final_file');
for i = 3:length(SubFold_fd)% start at 3 to skip . and ..
    ...
    if exist(inputFullFileName_fd,'file')==2
        ...
        if(temp_fd_percentage<=0.30)
            ...
            imgfile = dir([tempPath,'rp_sub-*txt']);
			%imgfile2 = dir([tempPath,'SpatialTemporalRegression_ica_c*-1.mat']);
